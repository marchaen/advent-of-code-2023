...
/**
 * Implementation of the solution for adventofcode 2023 day 04 part two
 *
 * # Problem description from aoc
 *
 ...
 * card 3, **`8`** instances of card 4, **`14`** instances of card 5, and
 * **`1`** instance of card 6. In total, this example pile of scratchcards
...
export function solvePartTwo(input: string): number {
    ...
    const cardCounters = Array(cards.length).fill(1);

    cards.forEach(card => {
        const matchingNumbers = card.got.filter((number) => card.winning.includes(number)).length;

        for (let offset = 0; offset < matchingNumbers; offset++)
            cardCounters[card.id + offset + 1] += cardCounters[card.id];
    });

    return cardCounters.reduce((sum, current) => sum + current, 0) - 1;
}
